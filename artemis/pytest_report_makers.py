...
def average_fallback_durations(journeys):
    total_start_fallback = 0.
    start_count = 0
    total_end_fallback = 0.
    end_count = 0
    ...
    for journey in journeys:
        ...
        if first_section["type"] == "street_network" or last_section["type"] == "street_network":
            start_or_end_count += 1

    average_start_fallback = total_start_fallback / (1. * start_count) if start_count > 0 else 0.
    average_end_fallback = total_end_fallback / (1. * end_count) if end_count > 0 else 0.
    average_fallback = (total_start_fallback + total_end_fallback) / (1. * start_or_end_count ) if start_or_end_count > 0 else 0.
...
def average_walking_duration(journeys):
    ...
    average_walking = total_walking_duration / (1. * count) if count > 0 else 0.
    return average_walking

def average_nb_pt_section(journeys):
    total_nb_pt_section = 0.
    count = 0
    for journey in journeys :
    ...
    return total_nb_pt_section / (1. * count) if count > 0 else 0.
...
def add_to_csv_report(ref_dict, resp_dict, test_name):
    ...
    ref_average_start_fallback, ref_average_end_fallback, ref_average_fallback = average_fallback_durations(refs)
    resp_average_start_fallback, resp_average_end_fallback, resp_average_fallback = average_fallback_durations(resps)

    start_fallback_variation = "{:+.0f}".format((resp_average_start_fallback - ref_average_start_fallback) * 100 / ref_average_start_fallback) if ref_average_start_fallback > 0. else ""
    end_fallback_variation = "{:+.0f}".format((resp_average_end_fallback - ref_average_end_fallback) * 100 / ref_average_end_fallback) if ref_average_end_fallback > 0. else ""
    fallback_variation = "{:+.0f}".format((resp_average_fallback - ref_average_fallback) * 100 / ref_average_fallback) if ref_average_fallback > 0. else ""


    ref_average_nb_pt_sections = average_nb_pt_section(refs)
    resp_average_nb_pt_sections = average_nb_pt_section(resps)
    nb_pt_section_variation = "{:+.0f}".format((resp_average_nb_pt_sections - ref_average_nb_pt_sections) * 100 /ref_average_nb_pt_sections) if ref_average_nb_pt_sections > 0. else ""

    ref_average_walking = average_walking_duration(refs)
    resp_average_walking = average_walking_duration(resps)
    walking_variation = "{:+.0f}".format((resp_average_walking - ref_average_walking) * 100 /ref_average_walking) if ref_average_walking > 0. else ""


    refs_use_same_line_consecutively = any(journey_use_same_line_consecutively(ref) for ref in ref_journeys)
    resps_use_same_line_consecutively = any(journey_use_same_line_consecutively(resp) for resp in resp_journeys)

    csv_report_path = os.path.join(config["RESPONSE_FILE_PATH"], "report.csv")

    nb_of_journey_variation = "{:+.0f}".format((len(resps) - len(refs)) * 100.0 / (1.0 * len(refs)))  if len(resps) != len(refs) else ""
    ...
    with open(csv_report_path, reading_mode) as csv_report:
        ...
        csv_report.write(
            ";".join([
                ...
                "{:.0f}".format(resp_average_start_fallback - ref_average_start_fallback),
                "{}".format(start_fallback_variation),
                "{:.0f}".format(ref_average_end_fallback),                
                "{:.0f}".format(resp_average_end_fallback),       
                "{:.0f}".format(resp_average_end_fallback - ref_average_end_fallback),          
                ...
                "{:.2f}".format(resp_average_nb_pt_sections - ref_average_nb_pt_sections),         
                "{}".format(nb_pt_section_variation),
                "{}".format(refs_use_same_line_consecutively) if refs_use_same_line_consecutively else "",
                "{}".format(resps_use_same_line_consecutively) if resps_use_same_line_consecutively else "",
                "\n"
            ])
        )
